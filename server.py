...
def get_user(user_id):
    ...
    if user.current_rental:
        transport = user.current_rental.transport
...
@app.route('/transports/<int:transport_id>/rent', methods=['PATCH'])
def rent_transport(transport_id):
    ...
    if user.rent_transport(transport):
        return jsonify({}), 200
    return jsonify({'error': 'Failed to rent'}), 400


@app.route('/users/<int:user_id>/return', methods=['PATCH'])
def return_transport(user_id):
